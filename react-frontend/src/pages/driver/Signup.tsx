...
export default function DriverSignup() {
  ...
  const handleStep1Submit = async (e: React.FormEvent) => {
    ...
    // Check if emergency contact is same as user mobile
    const driverMobile = formData.mobile.replace(/\D/g, '').slice(-10);
  ...
  const handleStep2Submit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const result = await driverApi.signupVerify({ ...formData, otp });
    ...
    <PublicLayout>
      <Box sx={{ minHeight: 'calc(100vh - 200px)', bgcolor: 'grey.50', py: { xs: 6, md: 8 }, px: 2 }}>
        ...
        <Box sx={{ textAlign: 'center', mb: 4 }}>
          ...
          <Typography variant="body1" sx={{ color: 'text.secondary' }}>
            Join HushRyd as a driver and start earning
          </Typography>
        </Box>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                ...
                <Grid container spacing={2}>
                  ...
                  helperText={referralCodeFromUrl 
                    ...
                    sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px' }, mt: 2 }}
                  />
