...
export default function CustomerSupportTickets() {
  ...
  const handleCreateTicket = async () => {
    ...
    try {
      ...
      await supportApi.createTicket({
        ...
        rideId: formData.rideId || undefined,
        subject: formData.subject.trim(),
        description: formData.description.trim(),
        priority: formData.priority,
      });
      setCreateDialogOpen(false);
      setFormData({ bookingId: '', rideId: '', subject: '', description: '', priority: 'medium' });
      loadTickets();
    } catch (err: any) {
  ...
  return (
    <PageContainer maxWidth="lg" title="Driver Support Tickets" subtitle="Raise a support ticket for website or app, trip, or earnings issues. Use complaints only for serious or unresolved concerns">
  ...
  <Typography variant="body2" sx={{ color: 'text.secondary', mb: 1 }}>
    Drivers should raise a support ticket for help with:
  </Typography>

  <Box component="ul" sx={{ pl: 3, mb: 2 }}>
    <li>Website or App not working or showing errors</li>
    <li>Login, OTP, or account access issues</li>
    <li>Trip assignment, cancellation, or trip status problems</li>
    <li>Earnings, incentives, or wallet-related issues</li>
    <li>Profile, vehicle, or document update help</li>
  </Box>

  <Typography variant="body2" sx={{ color: 'text.secondary' }}>
   ⚠️ Please raise a <strong>Support Ticket first</strong>.  
    If the issue is serious or not resolved, you may raise a <strong>Complaint</strong>.
  </Typography>
</StandardCard>
      {error && (
      ...
      {loading ? (
        ...
        <Box display="flex" flexDirection="column" gap={2}>
            {tickets.map((ticket) => (
              <Paper key={ticket.id} sx={{ p: 2, borderRadius: 2 }}>
                <Box display="flex" justifyContent="space-between" py={0.5} mt={0.5}>
                ...
                <Box display="flex" justifyContent="space-between" py={0.5} mt={1}>
                  <Typography variant="body2" color="text.secondary">
                    Actions
                  </Typography>
                  <IconButton
